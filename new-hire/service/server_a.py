...
def bank_details(document_details):
    ...
    for word in (text1.split()):
        if(not(accnum) and re.search("[0-9]{14}", word)):
            print(word)
            numbers = re.findall("[0-9]{14}", word)
            accnum = numbers[0]
            print(accnum)
        if(not(accnum) and re.search("[0-9]{13}", word)):
            print(word)
            numbers = re.findall("[0-9]{13}", word)
            accnum = numbers[0]
            print(accnum)
        if(re.search("[A-Z|a-z]{4}[0-9]{7}", word)):
            ifscode = word
        upper = word.upper()
        if(not(ifscode) and (upper =='CODE' or fl == 1 or upper == 'GODE' or upper == 'IFS' or upper == 'IFS CODE')):
            fl = 1
            if(word !=" " and word!="-" ):
	            ifscode = word
	            fl = 0
   ...
   #    if(len(number)==15):
    ...
    ref = cv2.imread("micr_e13b_referenc.png")
    #if (not(ref)):
